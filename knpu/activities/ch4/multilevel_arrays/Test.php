...
class Test extends AbstractSuite
{
    public function runTest(Result $result)
    {
        ...
        $petsElements = $this->getCrawler($output)->filter('.pet');
        if (count($petsElements) != 3) {
            $msg = sprintf('I expected to see 3 "div.pet" tags in the output, but I see "%s".', count($petsElements));
            if (count($petsElements) == 0) {
        ...
        $petElement = $petsElements->eq(0);
        $nameNode = $petElement->filter('.pet-name');
        $ageNode = $petElement->filter('.pet-age');
        $breedNode = $petElement->filter('.pet-breed');
