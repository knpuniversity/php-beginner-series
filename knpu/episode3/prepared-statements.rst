...
So let's use prepared statements already! First, change the ``query`` function
to ``prepare`` and put a ``:idVal`` where the id value should go. This returns
a ``PDOStatement`` object, so let's rename the variable too.
